...
namespace FacebookCommunityAnalytics.Crawler.NET.Service.Services
{
    ...
    {
        public ChromeDriver InitChromeDriver(ProxyModel proxy = null)
        {
            return (ChromeDriver) InitDriver(proxy);
        }

        private IWebDriver InitDriver(ProxyModel proxy = null)
        {
        ...
        public HtmlDocument LoadNewTab(ChromeDriver driver, string url)
        {
        ...
        public void CloseTabs(ChromeDriver driver)
        {
            ...
            for (var i = driver.WindowHandles.Count - 1; i > 0; i--)
            {
