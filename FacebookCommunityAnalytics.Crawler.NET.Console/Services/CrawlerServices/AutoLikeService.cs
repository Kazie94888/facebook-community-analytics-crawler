...
{
    public class AutoLikeService : CrawlServiceBase
    {
        public AutoLikeService(GlobalConfig globalConfig) : base(globalConfig)
        {
        ...
        public override async Task Execute()
        {
            ...
            try
            {
                ...
                //3. Do auto like
                var batchSize = GlobalConfig.CrawlConfig.BatchSize_Max_AutoLike;
                var threadCount = GlobalConfig.CrawlConfig.Crawl_MaxThread_AutoLike;
                ...
                System.Console.WriteLine($"{GetType().Name}: FOUND {autoPostFacebookNotDone.Count} auto like posts");
                ...
                await autoPostFacebookNotDone.Partition(batchSize).ParallelForEachAsync(async batch =>
                {
                    ...
                    var autoPosts = crawlSource.Where(dto => dto.NeedLike != dto.CurrentLike).ToList();
                    CRAWL:
                    try
                    {
                        accountProxy = accountProxyQueue.Dequeue();
                        var crawlResult = await AutoLike(accountProxy, autoPosts);
        ...
        private async Task<List<AutoPostFacebookNotDoneDto>> AutoLike(AccountProxyItem accountProxy, List<AutoPostFacebookNotDoneDto> autoPosts)
        {
            var likedPosts = new List<AutoPostFacebookNotDoneDto>();
                ...
                await using (browserContext.Browser)
                {
                    ...
                    try
                    {
                        ...
                        if (loginResponse.Success)
                        {
                            ...
                            System.Console.WriteLine(
                                $"====================={this.GetType().Name}: Trying to Auto Like for {autoPosts.Count} posts");
        ...
        private async Task DoCrawl(IPage page)
        {
            // ---------------- single post
            var selectorReact =
                "//descendant::div[@aria-label='Change Like reaction' or @aria-label='Thay đổi cảm xúc Thích' or @aria-label='Bày tỏ cảm xúc' or @aria-label='React']";
            var eleReact = await page.QuerySelectorAsync(selectorReact);
            await page.EvaluateAsync("eleReact => eleReact.scrollIntoViewIfNeeded(true)", eleReact);
            await page.WaitASecond();
            
            var selectorChangeLike =
                "//descendant::div[@aria-label='Change Like reaction' or @aria-label='Thay đổi cảm xúc Thích']";
            var eleChangeLike = await page.QuerySelectorAsync(selectorChangeLike);
            if (eleChangeLike == null)
            {
                var selectorLike = "//descendant::div[@aria-label='Thích' or @aria-label='Like']";
                var eleLike = await page.QuerySelectorAsync(selectorLike);
            
                if (eleLike != null)
                {
                    await eleLike.ClickAsync();
                }
            }
